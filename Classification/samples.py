...
def loadDataTrainingFile(filename, arrs, width, height):
  """
  Reads n data images from a file and returns a list of Datum objects.
  ...
  for num in arrs:
    counter = num
    temp = (counter-Amid) * DATUM_HEIGHT
    if temp != 0:
      temp = temp - DATUM_HEIGHT
    while temp != 0:
      fin.pop()
      temp = temp - 1
    data = []
    ...
    if len(data[0]) < DATUM_WIDTH - 1:
      # we encountered end of file...
      print "Truncating at %d examples (maximum)" % num
      break
    Amid = counter
    items.append(Datum(data, DATUM_WIDTH, DATUM_HEIGHT))
  """
...
def loadLabelsTrainingFile(filename, arrs):
  """
  Reads n labels from a file and returns a list of integers.
  """
  fin = readlines(filename)
  labels = []
  for num in arrs:
    if num > min(max(arrs),len(fin)):
      break
    elif fin[num-1] == '':
      break
    else:
      labels.append(int(fin[num-1]))
  return labels
