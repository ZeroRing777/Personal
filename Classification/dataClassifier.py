...
def readCommand( argv ):
  ...
  parser.add_option('-c', '--classifier', help=default('The type of classifier'), choices=['knn', 'nb', 'naiveBayes', 'perceptron', 'mira', 'minicontest'], default='knn')
...
USAGE_STRING = """
  ...
  EXAMPLES:   (1) python dataClassifier.py
                  - trains the default knn classifier on the digit dataset
...
def runClassifier(args, options):
  ...
  if(options.data=="faces"):
    rawTrainingData = samples.loadDataTrainingFile("facedata/facedatatrain", arr,FACE_DATUM_WIDTH,FACE_DATUM_HEIGHT)
    trainingLabels = samples.loadLabelsTrainingFile("facedata/facedatatrainlabels", arr)
    ...
    rawTrainingData = samples.loadDataTrainingFile("digitdata/trainingimages", arr,DIGIT_DATUM_WIDTH,DIGIT_DATUM_HEIGHT)
    trainingLabels = samples.loadLabelsTrainingFile("digitdata/traininglabels", arr)
  ...
  if((options.odds) & (options.classifier != "knn")):
